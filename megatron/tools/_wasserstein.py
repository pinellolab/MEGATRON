...
def _wasserstein_parallel(mat_clone,
                         mat_coord,
                         df_time,
    ...
    for i in range(num_clones):
        ...
        for j in range(i):
            ...
            coords_for_j = mat_coord[cells_in_j]
            time_for_j = df_time[cells_in_j]
            params = [cells_in_i, coords_for_i, time_for_i, cells_in_j, coords_for_j, time_for_j, t]
            dist_result = pool.apply_async(calc_dist, params)
            dist = dist_result.get()
            if dist == 0:
                num_noninform += 1
            distance_matrix[i][j] = dist
    #print("Out of " + str(math.comb(num_clones, 2)) + " clonal distances, " + str(num_noninform) + " are noninformative")
...
def calc_dist(params):
    dist = 0
    cells_in_i, coords_for_i, time_for_i, cells_in_j, coords_for_j, time_for_j, time_steps = params
    for t in time_steps:
        ts_i = np.where(time_for_i == t)[0]
        ts_j = np.where(time_for_j == t)[0]
        #print(ts_i.size)
